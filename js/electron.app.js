...
System.register("platform.js", [], function (_export, _context) {
	var remote, argv, baseURI;
	return {
		setters: [],
		execute: function () {
			remote = require("remote");

			_export("argv", argv = remote.process.argv.slice(2));
			...
			_export("baseURI", baseURI = "file://" + require("process").cwd() + "/");

			_export("baseURI", baseURI);

			function onOpen(callback) {
				require("electron").ipcRenderer.on("open", function (event, args, baseURI) {
					return callback(args, baseURI);
				});
			}
